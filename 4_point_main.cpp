...
PROGRAM_INFO("4-point correlation estimation.",
             "Does 4pcf correlation counts for a single matcher on a single node with optional jackknife resampling.");
...
PARAM_STRING_REQ("matcher_lower_bounds",
                 "The lower bound distances for the matcher, stored in a symmetric 4x4 matrix.", "l")
PARAM_STRING_REQ("matcher_upper_bounds",
                 "The upper bound distances for the matcher, stored in a symmetric 4x4 matrix.", "u")
...
int main(int argc, char* argv[])
{
  ...
  if (0 == resampler_str.compare("naive")) {
    ...
    if (0 == kernel_str.compare("general")) {
      ...
      NaiveResamplingDriver<Efficient4ptMatcher, GenericNptAlg<Efficient4ptMatcher>,
      NptNode, SingleResults>
      driver(data_mat,
             weights,
             random_mat,
