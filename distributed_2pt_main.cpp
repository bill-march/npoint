/**
 * @file distributed_2_point_main.cpp
 * @author Bill March (march@gatech.edu)
 *
 * Does general distributed 2-point computation.
 */
...
PROGRAM_INFO("Distributed 2-point correlation estimation.", "Does 2pcf correlation counts for a single matcher on many nodes with optional jackknife resampling.");
...
PARAM_STRING("resampler", "Select the type of resampling to use.  Only \"efficient\" is supported for now.", "s", "efficient");
...
int main(int argc, char* argv[])
{
  ...
  if (world.rank() == 0) {
    ...
    //mlpack::Log::Info << "Filling in data all weights.\n";
  ...
  if (0 == resampler_str.compare("naive")) {
    ...
    if (0 == kernel_str.compare("general")) {
      ...
      DistributedNptDriver<NaiveResamplingDriver<Efficient2ptMatcher, GenericNptAlg<Efficient2ptMatcher>,
      NptNode, SingleResults>, SingleResults>
      driver(world,
             data_mat,
             data_weights,
  ...
  if (0 == resampler_str.compare("efficient")) {
    ...
    if (0 == kernel_str.compare("general")) {
      ...
      DistributedNptDriver<EfficientResamplingDriver<Efficient2ptMatcher, GenericNptAlg<Efficient2ptMatcher>,
      NptNode, SingleResults>, SingleResults>
      driver(world,
             data_mat,
             data_weights,
