...
static __inline__ uint32_t npt_popcount(uint16_t x) {
	return __builtin_popcount(x);
}

static __inline__ uint32_t npt_popcount(uint8_t x) {
	return __builtin_popcount(x);
}
	...
	const unsigned char *satisfiabilityLUT)
{
	...
	for (int i = 0; i < numPointsA; i += 1) {
		...
		for (int j = 0; j < numPointsB; j += 1) {
			...
			switch (sat0AB + sat1AB * 2 + sat2AB * 4) {
          // note that these don't work on the mac, not sure why
          //case 0b000:
				case 0:
          break;
        case 1:
          //case 0b001:
					for (int k = 0; k < numPointsC / NPT_UNROLL_FACTOR; k += 1) {
						...
						count3_addition += npt_popcount((sat1AC & sat2BC) | (sat2AC & sat1BC));
					}
					break;
        case 2:
          //case 0b010:
					for (int k = 0; k < numPointsC / NPT_UNROLL_FACTOR; k += 1) {
						...
						count3_addition += npt_popcount((sat0AC & sat2BC) | (sat2AC & sat0BC));
					}
					break;
        case 3:
          //case 0b011:
        ...
        case 4:
          //case 0b100:
					for (int k = 0; k < numPointsC / NPT_UNROLL_FACTOR; k += 1) {
						...
						count3_addition += npt_popcount((sat0AC & sat1BC) | (sat1AC & sat0BC));
					}
					break;
        case 5:
          //case 0b101:
