/**
 * @file efficient_cpu_matcher.hpp
 * @author Bill March (march@gatech.edu)
 *
 * Header for 3-point correlation matcher that uses the efficient CPU kernels 
 * from the supercomputing paper.
 */

#ifndef _MLPACK_NPOINT_MATCHERS_EFFICIENT_CPU_MATCHER_HPP_
#define _MLPACK_NPOINT_MATCHERS_EFFICIENT_CPU_MATCHER_HPP_
...
namespace npoint_mlpack {
  ...
  /**
   * A 3-point correlation matcher that uses the efficient CPU kernels 
   * from the supercomputing paper.
   */
  class EfficientCpuMatcher {
  ...
  private:
    ...
    double lower_bounds_sqr_ptr_[3];
    double upper_bounds_sqr_ptr_[3];
    ...
    //! We use the same bit field interpretation for both the base case 
    //! (in the efficient kernel code) and the pruning steps
    //! These are the masks for permutations that may satisfy the matcher
    unsigned short valid_masks_[6];
  ...
  public:  
     ...
     */
    EfficientCpuMatcher(const std::vector<arma::mat*>& data_in, 
     ...
     */
    EfficientCpuMatcher(const EfficientCpuMatcher& other, bool is_copy = false);
     ...
     */
    void SumResults(const EfficientCpuMatcher& left_matcher, 
