/**
 * @file distributed_angle_2pt.cpp
 * @author Bill March (march@gatech.edu)
 *
 * Takes in an angle matcher arguments and computes the 2pt counts for each 
 * leg of each triangle, using the distributed (MPI) code. 
 *
 * The results take the form of r1[i], r2[i], r3[i][j], where i iterates through
 * values of r1 and j through values of theta.  All the thetas for a given r1
 * appear before the next r1.
 */
...
PROGRAM_INFO("Distributed angle matcher 2-point correlation estimation.",
             "Computes the 2pcf raw correlation counts for an angle matcher. "
             "These are the 2pcf counts for each leg of each triangle that "
             "appears in the angle matcher. "
             "\n"
             "The results take the form of r1[i], r2[i], r3[i][j], where i iterates through "
             "values of r1 and j through values of theta.  All the thetas for a given r1 "
             "appear before the next r1.");
...
int main(int argc, char* argv[])
{
  ...
  if (world.rank() == 0) {
    ...
    if (CLI::HasParam("random")) {
      ...
      mlpack::Log::Warn << "You should reuse the randoms from your 3pt computation.\n";
    }
  ...
  MatcherArguments matcher_args = angle_args.Generate2ptMatchers();
  
  world.barrier();
  
  Timer::Start("compute_time");
  
  DistributedNptDriver<EfficientResamplingDriver<UnorderedEfficientMultiMatcher,
